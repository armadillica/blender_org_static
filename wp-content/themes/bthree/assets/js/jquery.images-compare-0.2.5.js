...
;(function ($, window) {
    ...
    function ImagesCompare(element, options) {
        ...
        function onImagesLoaded() {
            ...
            images.each(function() {
                ...
                if ($(this)[0].complete) {
                    ...
                    // Image loading / error
                    $(this).on('load loadedmetadata', function() {
                        elementsLoaded++;
                        onImageLoaded();
                    });
        ...
        function patchSize() {
            var imgRef = backElement.find('img, video').first();

            // XXX Pablo
            if (imgRef.prop('nodeName') == 'VIDEO'){
                setSize(imgRef.width(), imgRef.height(), imgRef[0].videoWidth, imgRef[0].videoHeight);
            } else {
